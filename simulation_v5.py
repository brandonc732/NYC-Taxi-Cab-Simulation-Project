...
class SwapStation:
    ...
    """
    create an array of cabs in storage.
     
    They will simply be stored as a numpy array of times the cab is available at in seconds
    """
    def __init__(self, num_cabs, recharge_hours, minimum_seconds_till_end=3600):

        self.cabs = np.zeros(num_cabs)

        self.recharge_seconds = recharge_hours * 3_600

        self.minimum_seconds_till_end = minimum_seconds_till_end


    """
    Check if a cab if a cab is available at the current time in seconds

    If available, replace the entry at that index with the curr_time plus recharge seconds (we're assuming the cab is always dead)

    return True if one was available, false otherwise
    """
    def request_cab(self, curr_time_seconds, shift_end):
        
        # reject taxis near end of shift
        if (shift_end - curr_time_seconds) < self.minimum_seconds_till_end:
            return False

        # Find indices where cab is available
        available_mask = self.cabs <= curr_time_seconds

        if not np.any(available_mask):
            return False   # no cab available

        # Choose the first available cab
        cab_idx = np.argmax(available_mask)

        # Update its next availability time
        self.cabs[cab_idx] = curr_time_seconds + self.recharge_seconds
...
# NUMBA optimization for primary search
@njit(cache=True)
...
class TaxiFleet:
    ...
    """
    def __init__(self, taxi_shifts_df, taxi_range, trips_df, sampling_stuff, PU_to_DO_info, swap_station_locations, swap_station_capacity, recharge_hours, low_range_threshold=30, fleet_bank_size=20_000):
        ...
        self.setup_swap_stations(swap_station_locations, swap_station_capacity, recharge_hours)
    ...
    """
    Create a list the length of locationIDs

    for locationIDs in swap_station_locations, initialize a new swap station
    """
    def setup_swap_stations(self, swap_station_locations, swap_station_capacity, recharge_hours):
        
        self.swap_stations = []

        for i in range(265):
            if (i + 1) in swap_station_locations:
                # create swap station
                self.swap_stations.append(SwapStation(swap_station_capacity, recharge_hours))
            
            else:
                # append a nothing indicator
                self.swap_stations.append(None)
    ...
    def assign_trip(self, pickup_time_seconds, pickup_location, duration, distance, dropoff_location):
        ...
        if new_range <= self.low_range_threshold:
            ...
            # check if swap station is in location
            if self.swap_stations[dropoff_idx]:        # None if no swap station exists

                # check if swap station has available car
                swap_available = self.swap_stations[dropoff_idx].request_cab(new_time_available, shift_end)     #current time (after trip) and taxi's shift end      # this also rejects if taxi is close to end of shift 

                if swap_available: 
                    # swap was successful so set to full range
                    new_range = self.taxi_range
                
                else:
                    # swap was unsuccessful, so log as low threshold event
                ...
                # no swap station, so log as low threshold event
                self.taxi_below_threshold.append((new_time_available,  # current time (after trip)
                                                taxi_id,             # taxi id
                                                dropoff_idx + 1,        # current location (after dropoff)
                                                new_range,                            # current range
...
# For doing back logged trips, we can just process them at the time of the next trip and 
# just check if they match a taxi within a set time limit, and if they don't, 
# we can remove them if the current time is also beyond the time limit
...
def main(test_folder_arg):
    ...
    back_filled_trips_export_path = test_folder / "back_filled_trips.parquet"
    ...
    # swap station info
    swap_station_locations = config_dict['swap_station_locations'] # list of locationIDs
    swap_station_capacity = config_dict['swap_station_capacity']
    recharge_hours = config_dict['recharge_hours']
    ...
    fleet = TaxiFleet(
        ...
        swap_station_locations=swap_station_locations,
        swap_station_capacity=swap_station_capacity,
        recharge_hours=recharge_hours
    )
    ...
    # convert data
    first_takens = np.array(takens)
    back_takens = np.array(b_trip_takens)
    ...
    if len(fleet.primary_rejects_logs) > 0:
        ...
        del fleet.primary_rejects_logs
        gc.collect()
    else:
    ...
    if len(fleet.secondary_rejects_logs) > 0:
        ...
        del fleet.secondary_rejects_logs
        gc.collect()
    else:
    ...
    del fleet.taxi_below_threshold
    gc.collect()
    
    back_filled_trips_df = pd.DataFrame(back_filled_trips)
    ...
    if len(primary_rejects_df) > 0:
        primary_rejects_df['request_time'] = pd.to_datetime(primary_rejects_df['request_time'], unit='s', origin='2023-01-01')
        primary_rejects_df['shift_end'] = pd.to_datetime(primary_rejects_df['shift_end'], unit='s', origin='2023-01-01')
    
    if len(secondary_rejects_df) > 0:
        secondary_rejects_df['request_time'] = pd.to_datetime(secondary_rejects_df['request_time'], unit='s', origin='2023-01-01')
        secondary_rejects_df['shift_end'] = pd.to_datetime(secondary_rejects_df['shift_end'], unit='s', origin='2023-01-01')
    ...
    if len(back_filled_trips_df) > 0:
        back_filled_trips_df['PU'] = pd.to_datetime(back_filled_trips_df['PU'], unit='s', origin='2023-01-01')
        back_filled_trips_df['duration'] = back_filled_trips_df['duration'] / 3600
    ...
    back_filled_trips_df.to_parquet(back_filled_trips_export_path, index=False)
    ...
    # print takens stats
    filled = np.sum(first_takens == 1) + np.sum(back_takens == 1)
    unfilled = np.sum(back_takens == 0)
    print("number of non filled rides:", unfilled)
    print("percentage:", filled / (filled + unfilled))


    # write to save file
    with open(sim_time_export, "w") as f:
        f.write(f"Simulation finished in {end - start} seconds.\n")
        f.write(f"number of non filled rides: {unfilled}\n")
        f.write(f"percentage: {filled / (filled + unfilled)}\n")
