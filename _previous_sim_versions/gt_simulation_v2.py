...
def primary_search_numba(
    ...
    while taxi_bank[pickup_idx, j, 0] < gravestone + 1:

        # haven't reached end indication

        if (taxi_bank[pickup_idx, j, 0] <= pickup_time_seconds and
            taxi_bank[pickup_idx, j, 3] > pickup_time_seconds and
            taxi_bank[pickup_idx, j, 1] >= tg_distance):
            
            return pickup_idx, j, ib_miles, ib_time
...
def secondary_search_numba(
    ...
    while connecting_DOs[pickup_idx][i] > 0:
        ...
        # search through taxi's in location array until reach np.inf
        j = 1 # start after first marker
        while taxi_bank[DO_idx, j, 0] < gravestone + 1:

            # haven't reached end indication

            if (taxi_bank[DO_idx, j, 0] <= pickup_time_seconds and
                taxi_bank[DO_idx, j, 3] > pickup_time_seconds and
                taxi_bank[DO_idx, j, 1] >= tg_distance):
                
                return DO_idx, j, ib_miles, ib_time
...
class TaxiFleet:
    ...
    def clean_cab_array(self, curr_time_seconds):
        ...
        taxi_bank_cleaning_numba(current_time_seconds = curr_time_seconds,
    ...
    def find_taxi_for_trip(self, pickup_time_seconds, pickup_idx, distance):
        ...
        DO_idx, match_idx, ib_dist, ib_time = primary_search_numba(pickup_idx=pickup_idx,
        ...
        # TODO: maybe add logging for taxi rejects from out of range
        ...
        DO_idx, match_idx, ib_dist, ib_time = secondary_search_numba(pickup_idx=pickup_idx,
                                                                     pickup_time_seconds= pickup_time_seconds,
                                                                     trip_distance= distance,
    ...
    def assign_trip(self, pickup_time_seconds, pickup_location, duration, distance, dropoff_location):
        ...
        if new_time_available >= shift_end:
            ...
            #TODO: maybe add logger for total distance traveled by taxis being removed
            pass
...
def main(test_folder_arg):
    ...
    if len(rejects_df) > 0:
        rejects_df['request_time'] = pd.to_datetime(rejects_df['request_time'], unit='s', origin='2013-01-01')
        rejects_df['shift_end'] = pd.to_datetime(rejects_df['shift_end'], unit='s', origin='2013-01-01')
...
"""
    ...
    def sample_in_between_trip(self, starting_loc_idx, ending_loc_idx):
        
        start = self.in_between_starts[ending_loc_idx, starting_loc_idx]
        length = self.in_between_lengths[ending_loc_idx, starting_loc_idx]

        #sample_index = self.rng.integers(length)
        sample_index = np.random.randint(length)
...
"""
onnecting_DOs = self.PU_to_DO[pickup_location - self.PU_to_DO_offset] # list, not numpy array

        
        for DO_location in connecting_DOs:
            # get cab list at connecting dropoff 
            taxi_list_idx = DO_location - self.cab_indexing_offset

            stop = self.next_location_taxi_index[taxi_list_idx]

            if stop <= 1: # skip if there's no taxis in the array
                continue
            
            location_taxis = self.taxi_bank[taxi_list_idx, :stop]


            times = location_taxis[:, TIME]
            ends  = location_taxis[:, END]

            cand = (times <= pickup_time_seconds) & (ends > pickup_time_seconds)
            ...
            # Fast scalar check on the *first* feasible taxi
            idx = rel
            if location_taxis[idx, RANGE] >= td:
                self.search_secondary_time += time.time() - secondary_time_start
                return taxi_list_idx, idx, ib_dist, ib_time

            # Phase B: only if needed, do the full range mask (still restricted to [1:stop])
            ranges = location_taxis[:, RANGE]
            full = cand & (ranges >= td)

            # We already tested rel, so start after it
            tail = full[rel:]
            rel2 = int(np.argmax(tail)) if tail.size else 0
            if tail.size and tail[rel2]:
                self.search_secondary_time += time.time() - secondary_time_start
                return taxi_list_idx, (rel + 1 + rel2), ib_dist, ib_time


            
            # do a full boolean check on all of the taxis at the connection location
            full_valid = ((location_taxis[:, TIME] <= pickup_time_seconds) & 
                          (location_taxis[:, END] > pickup_time_seconds) & 
                          (location_taxis[:, RANGE] >= target_distance))
            
            full_match_idx = np.argmax(full_valid)

            if full_match_idx != 0:
                self.search_secondary_time += time.time() - secondary_time_start
                return taxi_list_idx, full_match_idx, ib_dist, ib_time
            
            # TODO: log taxi rejections
"""